...
const CommonChat = ({ userType = "instructor" }) => {
  ...
  // Load messages for selected person
  const loadMessages = async (courseId, personId) => {
    try {
      setLoading(true);
      const res = await fetchMessagesApi(courseId, personId);
      if (res?.data?.data) {
  ...
  useEffect(() => {
    if (selectedPerson?.courseId && selectedPerson?.studentId) {
      loadMessages(selectedPerson.courseId, selectedPerson.studentId);
    }
  }, [selectedPerson]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const res = await sendMessageApi({
        receiverId: selectedPerson.studentId,
        courseId: selectedPerson.courseId,
        message: messageText,
      });
  ...
  const panelLabel =
    userType === "instructor" ? "Student Messages" : "Instructor Messages";
      ...
      <div className="w-80 flex flex-col shadow-xl border-0 overflow-hidden rounded-2xl bg-white">
        <div className="pb-4 px-4 pt-4" style={{ backgroundColor: "#142C52" }}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-white text-lg font-bold">{panelLabel}</h2>
          </div>
          ...
          {filteredConversations.length > 0 ? (
            filteredConversations.map((conv) => (
              <div
                ...
                onClick={() => setSelectedPerson(conv)}
                className={`p-4 rounded-xl cursor-pointer transition-all duration-200 ${
                  selectedPerson?.studentId === conv.studentId
            ...
            >
              <div>
                <h3 className="text-white text-lg font-bold">
                  {selectedPerson.studentName}
                </h3>
                <p className="text-gray-300 text-sm mt-1">
                  {selectedPerson.courseName}
                </p>
              </div>
                ...
                <div className="space-y-3">
                  {messages.map((msg, index) => {
                    const isCurrentUser =
                      ...
                      msg.senderId !== selectedPerson.studentId;
          ...
          >
            <div className="text-center">
              ...
              <p className="text-gray-600 text-lg font-semibold">
                Select a {userType === "instructor" ? "student" : "teacher"} to
                start chatting
              </p>
